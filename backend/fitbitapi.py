...
def get_human():
	...
	for entry, sedEntry, lightEntry, fairEntry, veryEntry in zip(data["activities-steps"], 
		...
		output_dict = {"date": entry["dateTime"], "activity": int(entry["value"]), "target": goal_steps, "min_play": int(veryEntry["value"]), "min_active": int(fairEntry["value"]) + int(lightEntry["value"]), "min_rest": int(sedEntry["value"])}
